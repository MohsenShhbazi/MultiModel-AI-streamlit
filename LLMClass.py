...
from google.api_core import retry as rt
...
from tenacity import retry, wait_exponential, stop_after_attempt, retry_if_result, retry_if_exception_type, retry_if_exception
import os
...
Summarize the provided table or text concisely.

Do not include any introductory phrases or additional comments.  Respond with only the summary.
...
is_retriable = lambda e: (isinstance(e, genai.errors.APIError) and e.code in {429, 503}) # type: ignore


class EmbeddingFunction(EmbeddingFunction): # type: ignore


  @rt.Retry(predicate=is_retriable)
  def embed_query(self, text : str) -> List[float]:
    ...
    return [e.values for e in response.embeddings][0] # type: ignore
...
class LangChainLLMByGoogle:
  ...
  def __init__(self,api_key, kind='pdf', assembly_api=None):
    self.api_key = api_key
    self.assembly_api = assembly_api
    self.google = GoogleGenerativeAI(model='gemini-1.5-flash', google_api_key=self.api_key)
    self.llm = ChatGoogleGenerativeAI(model="gemini-1.5-flash",google_api_key=self.api_key) # type: ignore
    ...
    chroma_runnable = RunnableLambda(lambda x: self.chroma.similarity_search(x, k=4)) # type: ignore
    ...
    chroma_runnable_pro = RunnableLambda(lambda x: self.chroma.similarity_search(x, k=4)) # type: ignore
  ...
  def evaluate(self, query):
    ...
    if target[0] == "SAME":
      output = """**both are same**\n\n{target}\n\n**AI answer** \n\n{answer_1}""".format(target=target[1],answer_1=answer_1)
      return output
    else:
      if target[0] == "A":
        name = "gemini 1.5 Pro"
        ...
        output = answer_2
      out = """**Model {name} output better resault** \n\n{target} \n\n**AI answer**\n\n{answer_1}""".format(name=name, target=target[1], answer_1=output)
    return out


  @rt.Retry(predicate=is_retriable)
  def __final_score__(self, query, answerA, answerB):
    evaluation_text = evaluation_prompt.format(
        query=query,
        answer_a=answerA,
        answer_b=answerB
    )
    reasoning_response = client.models.generate_content(model="gemini-2.0-flash",
                                          contents=evaluation_text)

    
    # Step 1: Get evaluation reasoning
    verbose_eval = reasoning_response.text
    ...
    structured_response = client.models.generate_content(model="gemini-2.0-flash",
                                                         contents=evaluation_text,
                                                         config=structured_output_config)

    structured_eval = structured_response.candidates[0].content.parts[0].text # type: ignore
    return structured_eval, verbose_eval
  ...
  def __generate_output_via_models__(self, query):
    google_model_pro = ChatGoogleGenerativeAI(model='gemini-1.5-pro', api_key=GOOGLE_API_KEY) # type: ignore
    rag_chain_pro_short_step =  google_model_pro | StrOutputParser()

    google_model_flash = ChatGoogleGenerativeAI(model='gemini-2.0-flash', api_key=GOOGLE_API_KEY) # type: ignore
  ...
  def __Parse_video_or_audio__(self, path):
    ...
    video_chunks = split_text.create_documents([transcript.text]) # type: ignore
...
#-------------------------------------------------Summaries Part-----------------------------------------------------
    ...
    table_summarize = self.__summaries_text__(tables_html)
...
#-------------------------------------------------Storing Part-----------------------------------------------------
  ...
  def __summaries_text__(self, chunks):
      google = GoogleGenerativeAI(model='gemini-2.0-flash-lite', api_key=GOOGLE_API_KEY) # type: ignore
      template = ChatPromptTemplate.from_template(prompt_text)
      summarize_chain = {"element": lambda x: x} | template | google | StrOutputParser()
      chunks_summarize = summarize_chain.batch(chunks) # type: ignore
